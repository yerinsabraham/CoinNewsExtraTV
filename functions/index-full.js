...
async function calculateReward(eventType, meta, userData) {
    try {
        ...
        if (rewardConfig && rewardConfig[eventType]) {
            ...
            // Fallback to default amounts - Updated CNE System for 1-10K tier
            const defaultRewards = {
                daily_checkin: 28.0,
                signup_bonus: 700.0,
                referral_bonus: 700.0,
                ad_view: 2.8,
                live_tv_watching: 7.0,
                other_videos_viewing: 7.0,
                social_follow: 100.0,
                chat_reward: 0.1,
                extra_ai_reward: 0.50,
                spotlight_reward: 2.8,
                quiz_per_question: 2.0,
                // Legacy event types for backward compatibility
                video_watch: 7.0,
                daily_airdrop: 28.0,
                live_stream: 7.0,
                quiz_completion: 20.0
            };
            baseRewardAmount = defaultRewards[eventType] || 0;
            console.log(`Using default reward: ${baseRewardAmount} CNE for ${eventType}`);
        }
...
// Cloud Function: Send Tokens to User (Finance Admin / Super Admin Only)
exports.sendTokensToUser = onCall({ cors: true }, async (request) => {
    try {
        const { userEmail, amount, reason } = request.data;
        const authUid = request.auth?.uid;
        ...
        // Check admin privileges and role
        const adminDoc = await db.collection('admins').doc(authUid).get();
        if (!adminDoc.exists) {
            throw new Error('Unauthorized - Admin access required');
        }
        ...
        const adminRole = adminData.role || 'updates_admin';
        
        // Only Super Admin and Finance Admin can send tokens
        if (adminRole !== 'super_admin' && adminRole !== 'finance_admin') {
            throw new Error('Unauthorized - Finance Admin or Super Admin access required');
        }
        ...
        // Convert to CME units (internal precision)
        const amountInUnits = Math.floor(amount * 100); // Assuming 2 decimal precision
        
        // Update user balance
        const userRef = db.collection('users').doc(userId);
        const currentBalance = userData.points_balance || 0;
        const currentAvailableBalance = userData.available_balance || 0;
        
        await userRef.update({
            points_balance: currentBalance + amountInUnits,
            available_balance: currentAvailableBalance + amountInUnits,
        ...
        await db.collection('rewards_log').add({
            ...
            status: 'COMPLETED',
            reason: reason || 'Admin token transfer',
            adminUid: authUid,
        ...
        return {
            success: true,
            message: `Successfully sent ${amount} CNE to ${userEmail}`,
            userId: userId,
            newBalance: (currentBalance + amountInUnits) / 100
        };
