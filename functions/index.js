...
/**
 * Admin function to send CNE tokens to a user
 * Only accessible by Super Admin and Finance Admin
 */
exports.sendTokensToUser = onCall({
  cors: true
}, async (request) => {
  ...
  try {
    // Get admin document to verify role
    ...
    // Only super_admin and finance_admin can send tokens
    if (adminRole !== 'super_admin' && adminRole !== 'finance_admin') {
      throw new HttpsError('permission-denied', 'Insufficient permissions to send tokens');
    }
    ...
    if (!userEmail || !amount) {
      throw new HttpsError('invalid-argument', 'userEmail and amount are required');
    }
    ...
    // Calculate amount in units (multiply by 100 for precision)
    ...
    // Log the transaction in rewards_log
    await db.collection('rewards_log').add({
      userId: userId,
      userEmail: userEmail,
      amount: amountInUnits,
      eventType: 'admin_transfer',
      reason: reason || 'Manual token transfer by admin',
      adminId: authUid,
    ...
    await db.collection('admin_actions').add({
      ...
      details: {
        ...
        reason: reason || 'Manual token transfer'
      },
      timestamp: admin.firestore.FieldValue.serverTimestamp()
    });
    ...
    return {
      success: true,
      userId: userId,
      newBalance: newBalance / 100, // Convert back to decimal
      message: `Successfully sent ${amount} CNE tokens to ${userEmail}`
    };
    ...
    throw new HttpsError('internal', 'Failed to send tokens: ' + error.message);
  }
});
